...
%>% # è la pipe del tidyverse, che ci permette di scrivere codice più leggibile
  |> # è la pipe nativa di R, introdotta in R 4.1, che fa la stessa cosa ma è più recente
  
### 2) DATASET DI PARTENZA -----------------------------------

# Usiamo iris, già presente in R
data(iris)
dir.create("data")  # creiamo una cartella per i dati
...
#metodo sconsigliato iris[, c("Sepal.Length", "Species")] oppure iris[,c(1,5)] perchè se cambiano le colonne prende altre colonne
# Posso cambiare l'ordine delle colonne
iris %>%
  select(Species, Petal.Length, Sepal.Length)

# Posso selezionare un intervallo di colonne con :
iris_mod<-iris %>%
...
# Posso rinominare al volo le colonne
iris %>%
...
# Solo Setosa
iris %>%
  filter(Species == "setosa")#"filtra" in inglese significa mantenere, quindi mantengo quella specie e si fa con ==
#rbase--> iris[iris$Species == "setosa", ]

# selezionare Petali lunghi maggiori di 5
iris %>%
  filter(Petal.Length > 5)

# Due condizioni insieme: AND, si selezionano solo i fiori virginia con i petali >5
iris %>%
  filter(Species == "virginica", Petal.Length > 5)

# Stessa cosa scritta in modo esplicito si mette la e commerciale &
iris %>%
  filter(Species == "virginica" & Petal.Length > 5)

# Condizione OR, si selezionano i fiori setosa o versicolor
iris %>%
  filter(Species == "setosa" | Species == "versicolor")

iris %>%
  filter(Species == "setosa" | Species == "versicolor") |> 
  filter(Petal.Length > 1.5) #seleziono solo i petali maggiori di 1.5 tra quelli setosa o versicolor

# Posso combinare filter() e select()
iris_mod<-iris %>%
  filter(Petal.Length > 5) %>%
  select(Species, Petal.Length, Petal.Width) #esiste già nell'environment iris_mod ed R sovrascrive

#metodo scorretto ##iris_f <-iris |> 
                   #filter(Petal.Length > 5) e poi crea un'altra linea di codice  iris_s<- son slect. se mettiamo un pipe possiamo unire le due cose e non creare due oggetti diversi
...
### 8) ALCUNE REGOLE IMPORTANTI --------RICORDA-------
...
# e body_mass_g < 5000
filter(penguins, flipper_length_mm > 200, body_mass_g < 5000) |> 
